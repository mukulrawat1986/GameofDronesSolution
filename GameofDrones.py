            ...
            for drones in myDrones:
                ...
                if drones.pos.x == self.targetList[count].x and drones.pos.y == self.targetList[count].y:
                    ...
                    if self.zones[zoneNumber].ownerId == self.myTeamId:
                        ...
                        # debug info
                        self.debug('Drone no. %s goes to zone %s at coordinates %s' % (count, zoneNumber, self.targetList[count] ))
                        ...
                        # debug info 
                        self.debug('Drone no. %s goes to zone %s at coordinates %s' % (count, zoneNumber, self.targetList[count])) 
                        self.debug('Reason: Drone has not reached destination and the destination is either neutral or enemies')
